...
$(document).ready(function () {
    ...
    $("#addProduto").click(function () {
        ...
        if ($("#select2-chosen-1").text() == '-- Selecione o Produto --') {
            alert('Por favor, selecione um Produto!');
        } else {
            if ($("#OrdemCompraOvalor").val() == '') {
                var PV = '0,0';
            } else {
                var PV = $("#OrdemCompraOvalor").val();
            }

            var unidade = PV.replace(".", "");
            var unidade = unidade.replace(",", ".");

            var qtdvl = parseFloat(unidade * $("#OrdemCompraOquantidade").val());
            var produtos = $("#OrdemCompraKit option:selected").text().split('   - ');
            $("#tableKit").append('<tr><td><input name="OrdemCompra[id][]" readonly value="' + produtos[0] + '" ></td><td style="width: 45%;"><input name="OrdemCompra[Produto][]" readonly value="' + produtos[1] + '" ></td><td><input name="OrdemCompra[quantidade][]" readonly value="' + $("#OrdemCompraOquantidade").val() + '" ></td><td><input name="OrdemCompra[valor][]" readonly value="' + PV + '" ></td><td><a href="javascript:void(0);" class="remPE btn btn-danger btn-xs" valor="' + qtdvl + '"><i class="glyphicon glyphicon-remove"></i> Remover</a></td></tr>');
            $("#tableKit").show();

            var geral = parseFloat($("#OrdemCompraVtotal").val()) + parseFloat(qtdvl);
            $("#OrdemCompraVtotal").val(geral);
    ...
    $("#tableKit").on('click', '.remPE', function () {
        ...
        var geral = parseFloat($("#OrdemCompraVtotal").val()) - parseFloat(unidade);
        $("#OrdemCompraVtotal").val(geral);
...
$('.aFornecedor').autocomplete({
    source: www_root + 'fornecedores/lista',
    select: function (event, ui) {
        $("#OrdemCompraFornecedoreId").val(ui.item.Fornecedore.id);
        $("#OrdemCompraEmpresa").val(ui.item.Fornecedore.nome);
        $("#OrdemCompraCidade").val(ui.item.Fornecedore.cidade);
        $("#OrdemCompraContato").val(ui.item.Fornecedore.contato);
        $("#OrdemCompraEmail").val(ui.item.Fornecedore.email);
        $("#OrdemCompraTelefone").val(ui.item.Fornecedore.telefone);
    ...
    var html = "<strong>[" + item.Fornecedore.id.replace(regEx, replaceMask) + "] " + item.Fornecedore.nome.replace(regEx, replaceMask) + "</strong>";
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $.each(data, function (index, value) {
                ...
                var STP = value['Pedido']['pendente'] == false ? '<span style="color:green">Concluido</span>' : '<span style="color:red">Pendente</span>';
                if (value['KitsPedido']['parcial'] == 1) {
                ...
                $("#tableResumo").append('<tr><td>' + value['Pedido']['id'] + '</td><td>' + value['Kit']['nome'] + '</td><td>' + value['KitsPedido']['placa'] + '</td><td>' + formatdataTime(value['KitsPedido']['created']) + '</td><td>R$ ' + formatReal(valor) + '</td><td>' + STP + '</td><td>' + caixa + '</td><td>' + entregue + '</td></tr>');
