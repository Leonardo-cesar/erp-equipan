...
class OcProdutosController extends AppController {
    ...
    public function edit($id = null) {
        if (!$this->OcProduto->exists($id)) {
            throw new NotFoundException(__('Produto inválida'));
        }
        if ($this->request->is(array('post', 'put'))) {
            if ($this->OcProduto->save($this->request->data)) {
                $this->Session->setFlash(__('O Produto foi editada com sucesso.'), 'default', array('class' => 'alert alert-success', 'role' => "alert"), 'OcProduto');
                return $this->redirect(array('action' => 'index'));
            } else {
                $this->Session->setFlash(__('Produto não pode ser salva, tente novamente.'), 'default', array('class' => 'alert alert-danger', 'role' => "alert"), 'OcProduto');
            }
        } else {
            $options = array('conditions' => array('OcProduto.id' => $id));
            $this->request->data = $this->OcProduto->find('first', $options);
        }
    }
