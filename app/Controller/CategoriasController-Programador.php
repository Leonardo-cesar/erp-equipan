...
class CategoriasController extends AppController {
    ...
    public function edit($id = null) {
        ...
        if ($this->request->is(array('post', 'put'))) {
            if ($this->Categoria->save($this->request->data)) {
                $this->Session->setFlash(__('A categoria foi editada com sucesso.'), 'default', array('class' => 'alert alert-success', 'role' => "alert"), 'Fornecedore');
                return $this->redirect(array('action' => 'index'));
            } else {
                $this->Session->setFlash(__('Categoria não pode ser salva, tente novamente.'), 'default', array('class' => 'alert alert-danger', 'role' => "alert"), 'Fornecedore');
            }
        } else {
            $options = array('conditions' => array('Categoria.' . $this->Categoria->primaryKey => $id));
